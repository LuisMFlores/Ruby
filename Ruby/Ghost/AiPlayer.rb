...
class Aiplayer < Player
    ...
    def guess(fragment, num_of_players, dictionary)
        super(fragment)
        new_frag = ""
        possible_moves = dictionary.keys.select { |key| key.start_with?(fragment) }
    ...
    def winning_move(fragment, possible_moves, num_of_players, dictionary)
        selected_possible_moves = possible_moves.select { |moves| (moves.length - 1 )% num_of_players != 0 && !dictionary.has_key?(fragment + moves[0]) }
